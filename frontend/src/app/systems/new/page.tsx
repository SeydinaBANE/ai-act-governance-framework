...
export default function NewSystemPage() {
  ...
  const {
    ...
    append: appendTech,
    remove: removeTech,
  } = useFieldArray({ control, name: "tech_stack" });
  const {
    fields: dataFields,
    append: appendData,
    remove: removeData,
  } = useFieldArray({ control, name: "data_types" });

  const mutation = useMutation({
    mutationFn: (data: FormValues) =>
      api.post<AISystem>("/systems", {
        ...data,
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Enregistrez un nouveau système dans le registre
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <textarea
                ...
                placeholder="Décrivez le système..."
              />
            </div>
            ...
            <div className="col-span-2">
              ...
              <textarea
                ...
                rows={2}
                className="input w-full resize-none"
                placeholder="À quoi sert ce système ?"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {dataFields.map((field, i) => (
                <div key={field.id} className="flex gap-2">
                  <input
                    {...register(`data_types.${i}.value`)}
                    className="input flex-1"
                    placeholder="CV, emails, données biométriques..."
                  />
                  {dataFields.length > 1 && (
                    <button
                      type="button"
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-3">
              <input
                type="checkbox"
                {...register("affects_persons")}
                className="h-4 w-4 rounded border-gray-300 text-blue-600"
              />
              <div>
                <span className="text-sm font-medium text-gray-700">
                  Affecte des personnes physiques
                </span>
                <p className="text-xs text-gray-400">
                  Les décisions ont un impact sur des individus
                </p>
              </div>
            </label>
          </div>
