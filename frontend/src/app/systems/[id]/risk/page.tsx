...
import { ArrowLeft, ArrowRight, CheckCircle, AlertTriangle, XCircle, AlertOctagon } from "lucide-react";
...
export default function RiskScorerPage() {
  ...
  const mutation = useMutation({
    mutationFn: () => api.post<RiskAssessment>(`/risk/assess/${id}`, { answers }),
    onSuccess: (data) => {
      setResult(data);
        ...
        <div className="max-w-2xl">
          <div className={`rounded-xl border-2 p-8 ${RISK_COLORS[riskKey]}`}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide opacity-70">Résultat AI Act</p>
            ...
            {result.ai_act_articles && result.ai_act_articles.length > 0 && (
              <div className="mb-4">
                <p className="text-xs font-semibold uppercase tracking-wide opacity-70 mb-2">Articles applicables</p>
                <div className="flex flex-wrap gap-2">
                  {result.ai_act_articles.map((a) => (
                    <span key={a} className="rounded bg-white bg-opacity-50 px-2 py-0.5 text-xs font-mono font-semibold">{a}</span>
                  ))}
                </div>
              </div>
            )}

            <div className="text-sm opacity-70">Score : {result.total_score} / 100</div>
          </div>

          {result.required_actions && result.required_actions.length > 0 && (
            <div className="mt-6 card p-6">
              <h3 className="text-sm font-semibold text-gray-900 mb-3">Obligations réglementaires ({result.required_actions.length})</h3>
              <div className="space-y-3">
                {result.required_actions.map((action, i) => (
                  <div key={i} className="flex gap-3 rounded-md bg-orange-50 p-3">
                    ...
                    <div>
                      <p className="text-sm text-gray-700">{action.obligation}</p>
          ...
          <div className="mt-6 flex gap-3">
            ...
            <button
              onClick={() => { setResult(null); setAnswers({}); setCurrentSection(0); }}
    ...
    <PageShell>
      <div className="max-w-2xl">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Évaluation AI Act</h1>
            <p className="text-sm text-gray-500 mt-0.5">Section {currentSection + 1} / {totalSections}</p>
          </div>
        </div>
        ...
        <div className="card p-6">
            ...
            {section.questions.map((q) => (
              <div key={q.id} className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                <div className="flex items-start gap-2 mb-2">
                  ...
                  {q.ai_act_ref && (
                    <span className="shrink-0 rounded bg-blue-50 px-1.5 py-0.5 text-xs font-mono text-blue-600">{q.ai_act_ref}</span>
                  )}
                </div>
                {q.triggers_prohibited && (
                  <p className="text-xs text-red-500 mb-2">⚠ Une réponse &quot;Oui&quot; entraîne une classification INTERDITE</p>
                )}
          ...
          <div className="mt-6 flex gap-3">
            ...
            {isLast ? (
              ...
              >
                {mutation.isPending ? "Analyse..." : "Obtenir l'évaluation AI Act"}
              </button>
            ) : (
              <button
                onClick={() => setCurrentSection(s => s + 1)}
          ...
          {!allQuestionsAnswered && (
            <p className="text-xs text-gray-400 text-center mt-2">Répondez à toutes les questions pour continuer</p>
          )}
        </div>
      </div>
    </PageShell>
