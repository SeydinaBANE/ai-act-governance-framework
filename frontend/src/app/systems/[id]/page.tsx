...
export default function SystemDetailPage() {
  ...
  const { data: assessments } = useQuery({
    ...
    queryFn: () =>
      api.get<{ items: RiskAssessment[]; total: number }>(
        `/risk/assessments/${id}`,
      ),
    enabled: !!id,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (status: SystemStatus) =>
      api.patch<AISystem>(`/systems/${id}`, { status }),
    onSuccess: () => {
  ...
  const riskKey =
    system.current_risk_category ?? latestAssessment?.risk_category;
  ...
  return (
    <PageShell>
      ...
      <div className="mb-6 flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {system.owner_team ?? "Équipe non renseignée"} · v
              {system.version ?? "N/A"}
            </p>
          </div>
        </div>
        <button
          onClick={() => {
            if (confirm("Supprimer ce système ?")) deleteMutation.mutate();
          }}
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-6">
                ...
                {editStatus !== null ? (
                  <>
                    <select
                      value={editStatus}
                      onChange={(e) =>
                        setEditStatus(e.target.value as SystemStatus)
                      }
                    ...
                    <button
                      onClick={() => updateMutation.mutate(editStatus)}
                  ...
                  <button
                    onClick={() => setEditStatus(system.status)}
          ...
          {latestAssessment?.required_actions &&
            latestAssessment.required_actions.length > 0 && (
