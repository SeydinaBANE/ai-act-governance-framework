...
export default function PIIScannerPage() {
  ...
  const textMutation = useMutation({
    mutationFn: () =>
      api.post<PIIScan>("/pii/scan/text", { text, confidence_threshold: 0.85 }),
    onSuccess: (data) => {
      setLastScan(data);
          ...
          <div className="flex border-b border-gray-200">
            {(["text", "file"] as const).map((tab) => (
              ...
              >
                {tab === "text" ? (
                  <>
                    <FileText className="h-4 w-4" /> Texte libre
                  </>
                ) : (
                  <>
          ...
          {lastScan && (
            <div className="card p-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`rounded-full px-2 py-0.5 text-xs font-medium ${RISK_COLORS[lastScan.risk_level ?? "none"]}`}
                >
              ...
              {lastScan.pii_found ? (
                <>
                  <div className="flex items-center gap-2 text-sm text-orange-700">
                    ...
                    {lastScan.findings.length} occurrence(s) de données
                    personnelles détectées
                  </div>

                  {lastScan.entity_summary && (
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(lastScan.entity_summary).map(
                        ([type, count]) => (
                          <span
                            key={type}
                            className="rounded-full bg-orange-50 border border-orange-200 px-3 py-1 text-xs font-medium text-orange-700"
                          >
                            {type} × {count}
                          </span>
                        ),
                  ...
                  <div className="max-h-48 overflow-y-auto space-y-2">
                    {lastScan.findings.slice(0, 20).map((f, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-3 rounded-md bg-gray-50 px-3 py-2 text-xs"
                      >
                        <span className="rounded bg-red-100 px-1.5 py-0.5 font-semibold text-red-700">
                          {f.entity_type}
                        </span>
                        <span className="text-gray-500">
                          confiance: {(f.score * 100).toFixed(0)}%
                        </span>
                        {f.context && (
              ...
              {scansHistory.items.map((scan) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span
                      className={`rounded-full px-1.5 py-0.5 text-xs ${scan.pii_found ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"}`}
                    >
